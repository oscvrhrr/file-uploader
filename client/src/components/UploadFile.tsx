...
const UploadFile = ({ toggle, driveId, onUploadSuccess }:UploadFileProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-5 z-40">
       <form onSubmit={(event) => { sendFile(event); toggle() }} className="border flex items-end justify-between flex-col bg-radixgray-100 border-radixgray-600 w-[20rem] h-[15rem]  bg-radixblue-100 mx-auto rounded-xl px-4 py-4 absolute top-1/4 left-3/4 transform -translate-x-1/2 -translate-y-1/2" action="/dashboard/file" method="post" encType="multipart/form-data">
